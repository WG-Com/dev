...
RUN yum -y swap -- remove fakesystemd -- install systemd systemd-libs \
    ...
    && cd /root && wget http://124.202.164.4/files/2195000009818739/cn2.php.net/distributions/php-7.1.0.tar.gz \
    && tar zxvf php-7.1.0.tar.gz && cd php-7.1.0 \
    && ./configure ./configure --enable-fpm --with-mysql=mysqlnd --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --enable-maintainer-zts --disable-fileinfo  --with-openssl --with-zip --with-mbstring --enable-mbstring=all --with-ssh2 --with-curl \
