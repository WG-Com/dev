...
RUN yum swap -- remove fakesystemd -- install systemd systemd-libs \
    ...
    && ./configure --enable-fpm --with-mysql=mysqlnd --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --enable-maintainer-zts --disable-fileinfo  --with-openssl --with-zip --with-mbstring --enable-mbstring=all --with-ssh2 --with-curl \
